 ...
 * effects — types only. The engine keeps its own runtime types for now; the
 * session core (R7) and later adapter/persistence layers will adopt these
 * canonical types. Do not import this module from React components until the
 * extraction wiring exists.
 */
...
export interface SessionExercise {
  ...
  /** Canonical Exercise identity: Prisma `Exercise.id`. Present for program-derived plans. */
  exerciseId?: string;
  /** Canonical Exercise identity: `Exercise.slug`. Present for program-derived plans. */
  slug?: string;
}
