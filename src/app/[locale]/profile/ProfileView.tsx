...
function InfoRow({icon: Icon, chip, label, value}: {icon: LucideIcon; chip: string; label: string; value: string}) { return <div className="flex items-center gap-3 px-4 py-3 sm:px-5"><span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-apple-fill ${chip}`}><Icon className="h-4 w-4" aria-hidden="true" /></span><span className="text-[15px] text-apple-label-secondary">{label}</span><span className="ms-auto min-w-0 truncate text-[15px] font-medium text-apple-label" dir="auto">{value}</span></div>; }
