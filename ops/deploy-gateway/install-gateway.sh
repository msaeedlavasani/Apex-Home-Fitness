#!/bin/sh
# install-gateway.sh — idempotent install/upgrade of the constrained Apex
...
# Keep the existing Production service identity while granting the daemon the
# explicit Beta deployment directory. No Production path or resource is
# broadened by this unit change.
...
[ "$VERSION" = "5" ] || { echo "gateway version verification failed (got: $VERSION)" >&2; exit 1; }
