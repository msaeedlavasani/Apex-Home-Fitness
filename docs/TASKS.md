...
| State | `AUTHORIZED — real product integration and Beta authorization recorded`; the product checkpoint is PASS, but the derived Beta deployment capability is BLOCKED because no canonical Beta path exists; RUN-5 remains downstream |
...
| Next selectable work | `NONE` — Beta authorization is satisfied, but the derived Beta deployment capability is BLOCKED on the missing canonical Beta target/workflow/gateway path |
...
```json
{
  ...
  "items": [
    ...
    {"id":"BETA-DEPLOYMENT-CAPABILITY","status":"BLOCKED","frozen":false,"readinessRule":"DAG_DERIVED","admissionRequired":true,"taskProfile":"RELEASE","parallelSafety":"SERIAL_ONLY","ownerVisualAcceptanceRequired":false,"verification":"BLOCKED","result":"BETA_DEPLOYMENT_PATH_UNAVAILABLE — repository has no canonical Beta workflow, target mapping, or gateway allowlist"},
