...
| Field | Value |
|---|---|
| Active task | `CP-03 Android model-delivery field incident` — same-origin official MoveNet artifact repair implemented; physical Android retest pending; no unrelated work |
| Profile | `CODE_NO_DEPLOY` |
| Branch | (none) |
| State | `FIELD INCIDENT OPEN` — CP-03 delivery repair is in governed delivery; physical Android retest is the remaining evidence gate; all unrelated work remains closed or gated |
...
| Next authorized task | Physical Android retest of the corrected CP-03 harness (OWNER/HUMAN evidence gate); no additional autonomous change without new field evidence |
| Pending owner review | CP-03 physical Android retest; TS-03 Production deletion acceptance; MG-09 Production apply; other gated items unchanged |
...
| STATUS | **REOPENED / FIELD INCIDENT OPEN 2026-09-07 — harness delivery repair implemented, physical Android retest pending.** Feasibility findings remain DECIDED (Approach A); one iPhone Chrome CriOS squat cell remains measured at 9/10 = 90%; all other matrix cells remain honestly NOT_MEASURED. The Android failure is now verified as HTTP 403 model delivery before inference on two phones, reproduced as a tfhub.dev → Kaggle redirect/access failure; official Apache-2.0 MoveNet TF.js v4 Lightning/Thunder artifacts are bundled same-origin in `scripts/pose-measurement/models/`. Harness smoke **48/48**; authoritative finding: `architecture/CP-03-MODEL-FETCH-CPU-FALLBACK.md`. Physical Android retest of the corrected harness is required before CP-03 can be considered closed. No Companion camera functionality is implemented; product implementation remains separately gated (CP-04/TS-02/CP-06/CP-07). |
