...
| ADMIN-AUTH-PROD-01 migration + login recovery | PASS after deterministic migration with the lockfile-pinned Prisma 6.19.3 CLI from a source-derived ops image (no npx), then fixing the standalone proxy same-origin check and the provisioning helper via PR #11; real-browser acceptance 22/22 | Production migrations must use the exact pinned Prisma CLI — never dynamic npx resolution; standalone Next.js rebuilds request.url from HOSTNAME/PORT, so same-origin checks must accept the configured public origin; validate the real browser journey through the public HTTPS surface | `PITFALLS/PRISMA-NPX-PRODUCTION-MIGRATION.md`, `PITFALLS/NEXTJS-STANDALONE-PROXY-SAME-ORIGIN.md`, `PRODUCTION_CHECKPOINTS.md` ADMIN-AUTH-PROD-01 | CONFIRMED |
