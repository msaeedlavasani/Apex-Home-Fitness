...
ACTIVE_TASK:                           CP-03 Android model-delivery field incident — same-origin artifact repair implemented; physical Android retest pending.
ACTIVE_TASK_PROFILE:                   CODE_NO_DEPLOY (research harness only; no Product/Production change)
AHF_EXECUTION_STATE:                   ACTIVE — CP-03 field incident reopened 2026-09-07 after two-phone HTTP 403 evidence; official MoveNet artifacts bundled same-origin; smoke 48/48; physical retest is the remaining evidence gate.
ACTIVE_BRANCH:                         fix/cp-03-self-host-movenet-assets (pending governed delivery)
...
NEXT_AUTHORIZED_TASK:                  Physical Android retest of the corrected CP-03 harness (OWNER/HUMAN evidence gate); no further autonomous change is authorized after delivery without new evidence.
NEXT_EXPECTED_BRANCH:                 fix/cp-03-self-host-movenet-assets → PR/CI/merge lifecycle
CURRENT_PHASE:                         CP-03 FIELD INCIDENT OPEN — same-origin model-delivery repair implemented; physical Android retest pending. CP-04/CP-06/TS-03 remain delivered or separately gated.
LAST_UPDATED:                          2026-09-07 — CP-03 Android field incident reopened: two-phone evidence verified HTTP 403 on the tfhub.dev MoveNet model.json delivery path, repeated on WebGL and CPU; official Apache-2.0 MoveNet v4 Lightning/Thunder artifacts bundled same-origin; smoke 48/48; physical Android retest remains the evidence gate; see `architecture/CP-03-MODEL-FETCH-CPU-FALLBACK.md`. No Product/Production/legal/persistence change.
```
