...
| Pose/form feasibility spike (CP-03 — DECIDED 2026-09-03: **Approach A MoveNet/TF.js, web-first, fully on-device; v1 HIGH-coverage movements; TEMPO_DRIFT + validated RANGE_OF_MOTION only; real-device measurement gate REQUIRED before product implementation**; published-evidence evaluation — MoveNet best Android-browser FPS (34 Pixel 5) vs BlazePose ~11–12; ML Kit/Apple Vision native-only; JMIR 2026 camera-placement dominance; Pūioio 98.89% on-device rep counting; bounded harness + protocol at `scripts/pose-measurement/` — FPS/rep/placement/battery on Android Chrome + iPhone Safari; no product camera functionality; harness REPAIRED 2026-09-04 (three rounds: #1 staged/classified startup + LIVE-VIEW verification + CPU fallback; #2 real-human tracking failure — null inference input `state.video` never assigned → `estimatePoses(null)` silently zero poses, + MoveNet px-space keypoints making the overlay draw off-canvas; fixed via explicit mirrored capture-canvas inference, pixel-aware overlay, frame-content sampler + pose telemetry + INPUT_NEAR_BLACK/FLAT/STRUCTURED_NO_POSE/POSES_OK audit classification; #3 first real-device export (iPhone squat, `scripts/pose-measurement/results/iphone-squat-diagonal200-2026-09-04.json`) — tracking healthy yet detected 0/0: v2 rep machine latched `phase='lost'` forever on the first gated-out frame; v3 removes the dead latch (dropout continuity + 1.5 s re-arm), measures both sides, adds per-trial angle/gating telemetry; thresholds unchanged; smoke 32/32 incl. deterministic rep-machine regression scenario E; records [`architecture/CP-03-HARNESS-REPAIR.md`](architecture/CP-03-HARNESS-REPAIR.md) + [`architecture/CP-03-TRACKING-REPAIR.md`](architecture/CP-03-TRACKING-REPAIR.md) + [`architecture/CP-03-REP-HEURISTIC-REPAIR.md`](architecture/CP-03-REP-HEURISTIC-REPAIR.md); gate OPEN awaiting Owner v3 retest) | [`architecture/CP-03-POSE-FEASIBILITY.md`](architecture/CP-03-POSE-FEASIBILITY.md), [`../../scripts/pose-measurement/README.md`](../../scripts/pose-measurement/README.md) |
