...
| S02-D | Client adoption: player plan + logs + snapshots carry canonical id (additive, versioned) | **D1 COMPLETE 2026-08-27** (propagation contract + pure seam). **D2 COMPLETE 2026-08-27** (client plan adopts optional `exerciseId`/`slug`; `id` step-local unchanged; logs/snapshots deferred) |
