...
// Brand helpers — the canonical identity is an opaque branded string.
const exId = (s: string) => s as ExerciseId;
...
test('SessionExercise carries canonical identity alongside plan position', () => {
  const exercise = {
    ...
    restSeconds: 30,
    exerciseId: exId('ex_01hx7k2m'), // canonical Exercise.id (branded)
    slug: exSlug('squat'), // canonical Exercise.slug (branded)
  ...
  const plan: readonly SessionExercise[] = [
    exercise,
    {id: 'pe-2', name: 'پوش‌آپ', sets: 2, exerciseId: exId('ex_02push'), slug: exSlug('push-up')},
...
test('sessionContracts module is pure: types only, no runtime dependencies', () => {
  ...
  // Type-only imports (erased at compile time) are allowed; VALUE imports are
  // not — the module must stay runtime-pure.
  assert.match(code, /\bimport type\b/, 'canonical identity types come from a type-only import');
  assert.doesNotMatch(code, /\bimport\b(?!\s+type)/, 'contracts must not import values');
